...
def get_distances(num_nodes):
    ...
    for i in range(1, num_nodes+1):
        for j in range(i+1, num_nodes+1):
            distance = float(
                input(f"enter the distance between nodes{i} and node{j}:"))
...
def tsp_optimal_drilling(distances):
    G = nx.Graph()
    G.add_weighted_edges_from((i, j, distances)
                              for (i, j), distances in distances.items())
    optimal_length, optimal_order = nx.approximation.traveling_salesman_problem(
        G, cycle=True)
...
if __name__ == "__main__":
    while True:
        ...
        try_again = input(
            "do you want to try again with different number of nodes?(yes/no):").lower()
        if try_again != "yes":
            break
